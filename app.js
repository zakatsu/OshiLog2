  ...
  app.innerHTML = `
    ...
    <section class="entry-section" aria-labelledby="entry-title">
      ...
      <form class="entry-form${editingLog ? " is-editing" : ""}" id="entry-form">
        ...
        <label class="entry-field">
          <span>都市</span>
          <input name="city" type="text" required list="city-options" placeholder="東京 / 横浜市 / 大阪府" value="${escapeHtml(editingLog?.city || "")}" />
...
function setupEntryForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!coordinates) {
      message.textContent = "この都市はまだ自動判定できません。主要都市名で入力してください。";
      return;
    }
